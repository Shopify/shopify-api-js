...
function mergeOrAddHandler(
  ...
  for (const index in webhookRegistry[topic]) {
    ...
    if (handler.deliveryMethod === DeliveryMethod.Http) {
      ...
      throw new InvalidDeliveryMethodError(
        `Can only add multiple handlers for a topic when deliveryMethod is Http. Please be sure that you used addHandler method for once after creating Shopify instance in your app.  Invalid handler: ${JSON.stringify(
          handler,
        )}`,
      );
    }
