...
describe('ShopifyWebhooks.Registry.register', () => {
  beforeEach(async () => {
    config.API_VERSION = ApiVersion.Unstable;
...
describe('ShopifyWebhooks.Registry.process', () => {
  ...
  beforeEach(async () => {
    ...
    config.API_SECRET_KEY = 'kitties are cute';
    config.API_VERSION = ApiVersion.Unstable;
  ...
  it('handles the request when topic is already registered', async () => {
    ...
    await request(app)
      .post('/webhooks')
      .set(headers({hmac: hmac(config.API_SECRET_KEY, rawBody)}))
      .send(rawBody)
  ...
  it('handles lower case headers', async () => {
    ...
    await request(app)
      ...
      .set(
        headers({
          hmac: hmac(config.API_SECRET_KEY, rawBody),
          lowercase: true,
        }),
      )
      .send(rawBody)
...
function assertWebhookRegistrationRequest(
  ...
  const address =
    !webhook.deliveryMethod || webhook.deliveryMethod === DeliveryMethod.Http
      ? `${config.HOST_SCHEME}://${config.HOST_NAME}${webhook.path}`
      : webhook.path;
  expect({
    method: Method.Post.toString(),
    domain: webhook.shop,
